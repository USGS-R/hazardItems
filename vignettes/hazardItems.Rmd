...
```{r, eval=FALSE}
update.packages(repos = "http://owi.usgs.gov/R")
```
...
<br />
<br />
An example workflow for a new real time storm would involve the following:


(@) a new `template` item is added from the <a href="https://marine.usgs.gov/coastalchangehazardsportal/publish/item/" target="_blank">mediation page</a> describing the incoming storm (manually performed by NACCH administrators</a>). 
- The item should be given the Item Type `Storm` which also makes available the checkbox for Active Storm. The Active Storm checkbox is how the red `Active Storm` sash is applied to the item's card. - Check the checkbox for Show Children and Ribbonable and save the template item.
- The NACCH administrator should also add the NHC NOAA Cone, Track and Points layers to the template item as children.

(@) the NACCH administrators note the new template item's itemId (usually a 7-9 character string, such as "CAQw7M1")

(@) the NACCH administrator should navigate to the <a href="https://marine.usgs.gov/coastalchangehazardsportal/publish/tree/">Tree Manipulation Service</a> and move the new Active Storm item under Uber in order for it to display at the top level on the Portal.
...
(@) the user calls `publishStorm("CAQw7M1","D:\\MKHData\\cch\\github\\hazardItems\\inst\\extdata\\Sandy_CIDA.zip")` from R with parameters for templateId and full shapefile path (don't forget to double escape the slashes on windows as in the example above).

-- every time the model is updated, to replace all items on the desired active storm, follow steps 4 to 7

Currently, items that are replaced are orphaned and can be found under the Tree Manipulation Service under Orphaned items. An improvement is planned that will place them under a parent item so they are more easily located. 

When a storm is no longer 'Active' the user should return to the mediation page, and uncheck the active storm checkbox. They can alter its location from the Tree Manipulation Service, to either orphan the record or move it under another card.
...
* a new `template` item is added from the <a href="https://marine.usgs.gov/coastalchangehazardsportal/publish/item/" target="_blank">mediation page</a> describing the incoming storm (manually performed by NACCH administrators</a>). The item should be given the Item Type `Storm` which also makes available the checkbox for Active Storm. The Active Storm checkbox is how the red `Active Storm` sash is applied to the item's card. Check the checkbox for Show Children and Ribbonable and save the template item.
...
* the `template` function is called to create specified items with the contents of the new layer. `template` accepts a modifier `allAttributes` as true or false, indicating whether you want to replace all the attributes for the layer in the event you are updating a previously added layer.
...
To Use:
```{r,eval=FALSE}
addLayer(system.file("extdata", "Sandy_CIDA.zip", package="hazardItems"))
```
